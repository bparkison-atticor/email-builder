...
acceptance_criteria:
  - criterion: "A dark-mode switch and a three-option client picker (Gmail / Outlook / Apple Mail) exist in .preview-header-left, after the existing #testDataSwitch."
    verification: "grep -n 'darkModeSwitch' index.html returns a role=\"switch\" span inside .preview-header-left; grep -n 'data-dm=' index.html returns exactly 3 buttons with values gmail, outlook, applemail. All three buttons are present and enabled — none carries a `disabled` attribute (grep -n 'data-dm' index.html shows no `disabled`)."
...
3. **Markup.** Inside `.preview-header-left`, after the `#testDataSwitch` span (closes ~line 739) and before the container's closing `</div>` (~line 740), add a `<span class="divider" aria-hidden="true"></span>`, then:
   - `<span class="switch" role="switch" aria-checked="false" tabindex="0" id="darkModeSwitch" title="Simulate a dark-mode email client in the preview (preview only)"><span class="track"></span>Dark mode</span>` — mirrors `#testDataSwitch` exactly, but with no `on` class (defaults OFF).
...
11. **Wire the controls.** After the `syncTestDataSwitch();` call (line 2625) — this is after `const previewStage` is declared at line 2600, so it is in scope:
    ```js
...
- **Reusing `createModuleToggle` for the switch.** Rejected: it hard-codes `localStorage` persistence under `emailBuilder.module.<id>`, and persistence was explicitly declined. Hand-rolling against the `#testDataSwitch`/`flipTestData` shape (minus the `setItem` call) is ~12 lines and carries no unwanted behavior.
